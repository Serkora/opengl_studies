...
class Material(object):
    ...
    def apply(self, face=GL_FRONT_AND_BACK):
        ...
        self.diffuse = list(self.diffuse)
        self.ambient = list(self.ambient)
        self.specular = list(self.specular)
...
#             print(type(group.vertices))
#             print(len(group.vertices))
...
#             print(type(group.triangles))
#            group.triangles = GLfloat(group.triangles)
            glInterleavedArrays(GL_T2F_N3F_V3F, 0, group.array)
            glDrawArrays(GL_TRIANGLES, GLint(0), GLsizei(int(group.triangles)))
        glPopAttrib()
        glPopClientAttrib()
    ...
    def __init__(self, filename, file=None, path=None):
        ...
        for line in open(filename, "r"):
            ...
            if values[0] == 'v':
                ...
                tex_coords.append([float(values[1]),float(values[2])])
...
#                     print(tex_coords[t_index])
#                     print(type(tex_coords[t_index]))
#                     print(list(tex_coords[t_index]))
                    vertex = list(tex_coords[t_index]) + \
                             list(normals[n_index]) + \
                             list(vertices[v_index]) 
...
#                     print("VERTEX: ",vertex)
                    ...
                    if i == 0:
                        v1 = vertex
                    vlast = vertex
#         print("LENGTH: ",len(group.vertices))
